...
function agregarMiProducto(imagen){
		...
		datosProducto.append("stockMin", stockMin);
		datosProducto.append("stockMax", stockMax);
	...
	$.ajax({
		...
		success: function(respuesta){
			...
			=============================================*/
				...
				$("#modalEditarProducto .detalleUnidad").tagsinput('items');
				...
				$("#modalEditarProducto .detalleCodigo").tagsinput('items');
			...
			$("#modalEditarProducto .stockMin").val(respuesta[0]["stockMin"]);
			$("#modalEditarProducto .stockMax").val(respuesta[0]["stockMax"]);
					...
					if($("#modalEditarProducto .tituloProducto").val() != "" && 
					   ...
					   $("#modalEditarProducto .pClavesProducto").val() != ""){
						...
						/*=============================================
					   	...
					   	var totalMax = parseInt($('#modalEditarProducto .stockMax').val());
					   	var totalActual = parseInt($('#modalEditarProducto .stock').val());

					   	if (totalActual > totalMax) {
					   		swal({
							    title: "El la cantidad ingresada en stock supera el monto máximo que puede haber",
							    type: "error",
							    confirmButtonText: "¡Cerrar!"
							});
					   		return;
					   	...
					   	if (totalActual < 1) {
					   		swal({
							    title: "Debe ingresar una cantidad mayor a la indicada en el campo Stock Mínimo",
							    type: "error",
							    confirmButtonText: "¡Cerrar!"
							});
					   		return;
			...
			function editarMiProducto(imagen){
				...
				var stockMin = $("#modalEditarProducto .stockMin").val();
				var stockMax = $("#modalEditarProducto .stockMax").val();
				...
				var finOferta = $("#modalEditarProducto .finOferta").val();
			  	...
			  	if(seleccionarTipo == "virtual"){
					...
					var detalles = {"Unidad": $("#modalEditarProducto .detalleUnidad").tagsinput('items'),							
					       			"Codigo": $("#modalEditarProducto .detalleCodigo").tagsinput('items'),
