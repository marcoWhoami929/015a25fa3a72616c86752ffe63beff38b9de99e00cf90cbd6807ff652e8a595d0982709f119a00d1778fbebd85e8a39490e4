  ...
  public function mostrarTabla(){	
	...
	for($i = 0; $i < count($ventas); $i++){
		...
		if($traerCliente["foto"] != "" && $traerCliente["modo"] == "directo"){
			...
			$imgCliente = "<img class='img-circle' src='http://localhost/eshop/".$traerCliente["foto"]."' width='60px'>";

		}else if($traerCliente["foto"] != "" && $traerCliente["modo"] != "directo"){

			$imgCliente = "<img class='img-circle' src='".$traerCliente["foto"]."' width='60px'>";

		}
		else{

			$imgCliente = "<img class='img-circle' src='vistas/img/usuarios/default/anonymous.png' width='60px'>";
		}
		...
		if($ventas[$i]["envio"] == 0 && $tipo == "virtual"){
			...
			$envio ="<button class='btn btn-danger btn-xs btnEnvio' idVenta='".$ventas[$i]["id_pedido"]."' etapa='1'>Preparando producto</button>";

		}else if($ventas[$i]["envio"] == 1 && $tipo == "fisico"){

			$envio = "<button class='btn btn-warning btn-xs btnEnvio' idVenta='".$ventas[$i]["id_pedido"]."' etapa='2'>Enviando el producto</button>";
		...
		$datosJson	 .= '[
			      		"'.($i+1).'",
			      		"'.$ventas[$i]["id_pedido"].'",
			      		"'.$ventas[$i]["cantidad_productos"].'",
			      		"'.$cliente.'",
			      		"'.$imgCliente.'",
			      		"$ '.number_format($ventas[$i]["total_pago"],2).'",
			      		"'.$tipo.'",
			      		"'.$envio.'",
			      		"'.$metodo.'",	
			      		"'.$email.'",
			      		"'.$ventas[$i]["direccion"].'",
			      		"'.$ventas[$i]["pais"].'",
			      		"<a href=\'pedido/' . $ventas[$i]["id_pedido"] . '\'>Descargar</a>",
			      		"'.$ventas[$i]["fecha"].'"
			      		],';
